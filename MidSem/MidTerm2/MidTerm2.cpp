...
{
    gCount++;
    std::string wordPhrase = "Tao la bo chung may, chung may la con tao ok?";
    std::string key = "Relations";
    std::string decoded;
    KeyProvider p(key, wordPhrase);
    VigenereForwardIterator iterEncode(key, wordPhrase, EVigenereMode::Encode);
    for (const char s : iterEncode) {
        decoded.push_back(s);
    }
    std::cout << decoded << std::endl;

    
    for (VigenereForwardIterator iterDecode = VigenereForwardIterator(key, decoded, EVigenereMode::Decode); iterDecode != iterDecode.end(); iterDecode++)
        std::cout << *iterDecode;
    std::cout << "\n";

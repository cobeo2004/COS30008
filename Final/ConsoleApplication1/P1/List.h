...
public:
    ...
    ~List() noexcept
    {
        std::shared_ptr<DoublyLinkedList<T>> lCurrent = fTail;
        fTail.reset();

        while (lCurrent != nullptr)
        {
            std::shared_ptr<DoublyLinkedList<T>> lPrev = lCurrent->fPrevious.lock();
    ...
    List(List &&aOther) noexcept : List()
    {
        swap(aOther);
    }
